...
module.exports= async (req, res) => {
    try {
        ...
        const {id, name, age,photos,gender,occupation,relations}=req.body;
        ...
        const character_details = await Character.update(
            { _id : id},
            { $set: {
                ...
                relations: relations || character_prev.relations
            }}
          )
      
      //if we have successfully entered details into character schema then success message is generated
      await res.status(200).send({success:true,message:"CHARACTER DETAILS UPDATED SUCCESSFULLY" ,data :character_details})
      ...
      console.log(error)
      res.status(200).send({success:false,message :"ERROR IN CHARACTER DETAILS UPDATION" , data : error})
    }
    finally {
